...
Repo 전체 조작은 `lazygit`, 한 파일 안에서 쭉 읽는 diff는 `GitUnifiedDiff`, split 리뷰는 `diffview`를 쓴다.

| 용도 | 도구 | 명령/키 |
|------|------|---------|
| gitui 스타일 TUI | `lazygit.nvim` + `lazygit` | `<leader>gg`, `:LazyGit` |
| 현재 파일 중심 lazygit | `lazygit.nvim` | `<leader>gf`, `:LazyGitCurrentFile` |
| 생략 없는 unified working tree diff | custom `GitUnifiedDiff` | `<leader>gd`, `:GitUnifiedDiff` |
| 생략 없는 unified branch diff | custom `GitUnifiedDiff` | `<leader>gD`, `:GitUnifiedDiff origin/main...HEAD` |
| split 기반 diffview | `diffview.nvim` | `<leader>gv`, `:DiffviewOpen origin/main...HEAD` |
| 현재 파일 히스토리 | `diffview.nvim` | `<leader>gh`, `:DiffviewFileHistory %` |

브랜치 전체 변경사항을 한 파일 안에서 생략 없이 보려면:

```vim
:GitUnifiedDiff origin/main...HEAD
```

`GitUnifiedDiff`로 열린 diff 탭은 `diff` syntax highlight를 강제 적용하고,
아래 quickfix에 파일 인덱스를 자동으로 연다. diff 자체는 split 없이 한 버퍼에 이어서 표시된다.

| 키 | 동작 |
|----|------|
| `<leader>g]` | 다음 unified diff hunk (`@@`) |
| `<leader>g[` | 이전 unified diff hunk (`@@`) |
| `]f` | 다음 diff 파일 |
| `[f` | 이전 diff 파일 |
| `<leader>go` | 파일 인덱스 quickfix 열기/닫기 |
...
| `<leader>gd` / `<leader>gD` | unified diff / unified branch diff |
| `<leader>gv` / `<leader>gh` | Diffview split / file history |
| `<leader>g]` / `<leader>g[` | unified diff hunk 이동 |
| `]f` / `[f` | unified diff 파일 이동 |
| `<leader>go` | unified diff 파일 인덱스 toggle |
