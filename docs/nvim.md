...
| format | `sqlfluff` via conform.nvim |
| DB UI | `vim-dadbod-ui` |

`nanotee/sqls.nvim`는 별도 `require("sqls").setup()` 모듈을 제공하지 않아 사용하지 않는다.
...
| 현재 브랜치/PR split diff | smart Diffview | `<leader>gD` |
| 현재 파일 히스토리 | `diffview.nvim` | `<leader>gh`, `:DiffviewFileHistory %` |

`<leader>gD`는 base branch를 하드코딩하지 않는다. 우선순위:

1. `gh pr view`의 PR base branch
2. `branch.<name>.mergeBase` / `branch.<name>.gh-merge-base`
3. 현재 upstream이 같은 브랜치명이 아닐 때 upstream
4. `origin/HEAD`
5. `origin/main`, `origin/master`, `main`, `master`

선택된 base로 `DiffviewOpen <base>...HEAD`를 실행한다.
